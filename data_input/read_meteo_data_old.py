...
def read_weather_data(
        general_info, path_input_file_meteo,
        path_input_file):
    """
    ...
    try:
        meteo_file = pd.read_excel(path_input_file_meteo, skiprows=[0])
    ...
    # CREATING A WINDSPEED DATAFRAME
    # Checking if windspeed data provided by client
    if not all(pd.isnull(array_info.loc[:, 'windspeed_column'])):
        # If given, use column number given in array info to fetch data
        meteo_windspeed = meteo_file.loc[
                          :, array_info.loc[:, 'windspeed_column'].values]
        # Using array info to get same multi-index column name
        meteo_windspeed.columns = array_info.index
        # Setting datetime series as index
        meteo_windspeed = meteo_windspeed.set_index(meteo_datetime['datetime'])

        # CONVERTING WINDSPEED DATAFRAME INTO A MULTI-INDEX DATAFRAME
        meteo_windspeed.columns = add_index_curve_level(
            meteo_windspeed.columns, 'W')

        # Adding meteo_windspeed df to meteo_data df
        if meteo_data.empty:
            # if windspeed not provided by client we assign it as empty df
            meteo_data = meteo_windspeed.copy()
        else:
            meteo_data = pd.concat([meteo_data, meteo_windspeed], axis=1)
